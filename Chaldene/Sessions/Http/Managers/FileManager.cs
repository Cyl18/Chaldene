...
using File = Chaldene.Data.Shared.File;
...
public partial class MiraiBot
{
    ...
    public async Task<File> UploadFileAsync(GroupId target, string filePath, string uploadPath = "/")
    {
        ...
        var url = $"{HttpScheme}://{Address.HttpAddress}/{HttpEndpoints.FileUpload.GetDescription()}";
    ...
    /// <param name="imageUploadTargets">上传类型</param>
    /// <returns>item1: 图片id，item2：图片url</returns>
    public async Task<ImageInfo> UploadImageAsync(Stream stream,
        ImageUploadTargets imageUploadTargets = ImageUploadTargets.Group)
    {
        var url = $"{HttpScheme}://{Address.HttpAddress}/{HttpEndpoints.UploadImage.GetDescription()}";
        ...
        var result = await url
            ...
            .PostMultipartAsync(x => x
                .AddString("type", imageUploadTargets.GetDescription())
                .AddFile("img", stream, $"{Guid.NewGuid():D}.png")).ConfigureAwait(false);
    ...
    {
        var url = $"{HttpScheme}://{Address.HttpAddress}/{HttpEndpoints.UploadVoice.GetDescription()}";
...
public record ImageInfo(string ImageId, string Url)
{
    ...
    {
        return new ImageMessage() {ImageId = imageInfo.ImageId};
    }
}